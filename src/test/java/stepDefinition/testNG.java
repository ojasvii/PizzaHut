...
public class testNG {
	...
	public void setUp() {
		...
		try {
			...
			logger.fail("Instantiate of the Chrome Browser Failed");
		}
	}
	...
	public void pizzaAddItemToBasketFlow(String address, String name, String mobile, String email) {
		...
		try {
			...
			Thread.sleep(5000);
			try {
				driver.findElement(pageObj.StartYourOrderWithTime()).click();
			} catch (Exception e) {
			}
		...
		// Validate product is added under Basket but checkout button price item is
		// still now showing.
		float TotalBasketPrice = Float
				.valueOf(driver.findElement(pageObj.TotalPriceAmount()).getText().replaceAll("₹", ""));
		boolean flag = resObj.isElementPresent(driver,
				pageObj.PriceOptionUnderCheckout("₹" + String.valueOf(TotalBasketPrice)));
		if (!flag) {
